...
这份文件是 @google/design.md 对 Notion 官网（notion.com）抓取生成的设计规格，
记录的是 Notion 的品牌配色、字体与营销页版式，著作权属于 Notion。
留在这里只作为「一份完整的设计规格长什么样」的格式参考。

织章自己的规格见仓库根目录的 DESIGN.md。配色、字体、装饰一律以那份为准，
不要把这里的紫色 CTA、海军蓝 hero 或 Notion-Sans 搬进产品。
-->
